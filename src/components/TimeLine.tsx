...
function useTimeline(blogs: Blog[], isMobile: boolean, isTablet: boolean) {
  // Transform blogs to timeline items
  const timelineData: TimelineItem[] = blogs.map((blog, idx) => ({
    id: idx + 1,
  ...
  useEffect(() => {
    if (isMobile) {
      setVisibleRange({ start: 0, end: Math.min(1, timelineData.length) });
    } else if (isTablet) {
      setVisibleRange({ start: 0, end: Math.min(2, timelineData.length) });
    } else {
      setVisibleRange({ start: 0, end: Math.min(4, timelineData.length) });
    }
  }, [isMobile, isTablet, timelineData.length]);
...
function TimelineStep({
  ...
  return (
    <motion.div
      ...
      exit="hidden"
    >
      <div className="relative group min-w-25 text-center mb-4">
      ...
      <div className="relative group mt-4 min-w-25 text-center">
        <motion.span
...
export default function Timeline({ blogs }: TimelineProps) {
  ...
  const {
    ...
    setActiveStep,
    visibleRange,
  } = useTimeline(blogs, isMobile, isTablet);
        ...
        >
          ...
          >
            {visibleTimelineData.map((step, index) => (
              <div key={`${step.id}-${visibleRange.start}-${visibleRange.end}`} className="flex items-center">
                <TimelineStep
                  step={step}
